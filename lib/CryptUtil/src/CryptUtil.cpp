...
String CryptUtil::rc4Paradox(String text, String key)
{
  text.trim();
  key.trim();
  unsigned char S[256]; // table
  String result = "";

  // populate 255 byte table
  ...
  for (unsigned char i = 0; i < text.length(); i++) {
    x2 = i & 255;
    y  = (S[x2] + y) & 255;
    unsigned char t = S[x2];
    S[x2]   = S[y];
    S[y]    = t;
    result += (char)(text[i] ^ S[(S[x2] + S[y]) % 256]);
  }
  char *tmp = makeDigest((const unsigned char *)result.c_str(),
                         result.length());
  String resultHexStr = tmp;
  free(tmp);
  resultHexStr.toUpperCase();
  return resultHexStr;
}

char * CryptUtil::makeDigest(const unsigned char *digest, int len)
{
...
# define GET(n) \
  SET(n)
#else // if defined(__i386__) || defined(__x86_64__) || defined(__vax__)
# define SET(n)                              \
  (ctx->block[(n)] =                         \
     (MD5_u32plus)ptr[(n) * 4] |             \
...
# define GET(n) \
  (ctx->block[(n)])
#endif // if defined(__i386__) || defined(__x86_64__) || defined(__vax__)

/*
 * This processes one or more 64-byte data blocks, but does NOT update
 * the bit counters.  There are no alignment requirements.
 */
const void * CryptUtil::body(void *ctxBuf, const void *data, size_t size)
{
...
void CryptUtil::MD5Init(void *ctxBuf)
{
  MD5_CTX *ctx = (MD5_CTX *)ctxBuf;
...
void CryptUtil::MD5Update(void *ctxBuf, const void *data, size_t size)
{
  MD5_CTX *ctx = (MD5_CTX *)ctxBuf;
...
void CryptUtil::MD5Final(unsigned char *result, void *ctxBuf)
{
  MD5_CTX *ctx = (MD5_CTX *)ctxBuf;
  MD5_u32plus used, free;
...
unsigned char * CryptUtil::md5Sum(const char *arg)
{
  MD5_CTX context;
...
unsigned char * CryptUtil::md5Sum(const char *arg, size_t size)
{
  MD5_CTX context;
  unsigned char *hash = (unsigned char *)malloc(16);
...
String CryptUtil::md5SumHex(String arg)
{
  unsigned char *hash = md5Sum(arg.c_str());
  char  *hashHex      = makeDigest(hash, 16);
  String hashHexStr   = String(hashHex);

  free(hash);
  free(hashHex);

  hashHexStr.toUpperCase();
  return hashHexStr;
}
