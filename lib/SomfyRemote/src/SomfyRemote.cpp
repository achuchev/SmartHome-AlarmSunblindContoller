...
uint16_t SomfyRemote::buildFrame(int      remote,
                                 byte     button,
                                 uint16_t rollingCode,
                                 byte    *frame) {
  frame[0] = 0xA7;             // Encryption key. Doesn't matter much
  frame[1] = button << 4;      // Which button did you press? The 4 LSB will be
                               // the checksum
  ...
  frame[1] |= checksum; // If a XOR of all the nibbles is equal to 0, the blinds
                        // will consider the checksum ok.

  PRINT_D("With checksum : ");
  ...
  PRINTLN_D();
  PRINT_D("Rolling Code  : ");
  PRINTLN_D(rollingCode);

  // We store the value of the rolling code in the EEPROM.
  return rollingCode + 1;
};

uint16_t SomfyRemote::sendButton(int remote, byte button,
                                 uint16_t rollingCode) {
...
// sendLevelChange
void SomfyRemote::sendLevelChange(byte initLevel, int duration1,
                                  int duration2) {
...
void SomfyRemote::sendCommand(byte *frame, byte sync) {
  if (sync == FRAME_KIND_FIRST) { // Only with the first frame send wake-up
                                  // pulse & silence
  ...
  for (byte i = 0; i < 56; i++) {
    sendLevelChange(((((frame[i / 8] >> (7 - (i % 8))) & 1) == 1) ? LOW : HIGH),
                    SYMBOL,
                    SYMBOL);
  }
